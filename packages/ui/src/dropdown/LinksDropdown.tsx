...
const LinksDropdown = ({ Title, links }: LinksDropdownProps) => {
  return (
    <div className="relative flex items-center gap-2 group cursor-pointer text-white">
      ...
      <ChevronDown />
      <div
        className={`invisible absolute left-[50%] translate-x-[-49%] ${links.length ? "translate-y-[15%]" : "translate-y-[40%]"}  top-[50%] z-50 flex flex-col items-center rounded-md bg-slate-900 p-4 text-white opacity-0 transition-all duration-200 group-hover:visible  group-hover:opacity-100 lg:w-[300px] `}
      >
